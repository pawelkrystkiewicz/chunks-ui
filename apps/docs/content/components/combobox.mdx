...
All compound parts accept the same props as their underlying Base UI [`Combobox`](https://base-ui.com/react/components/combobox#api-reference) counterparts. `Combobox.Control` is a plain `<div>` wrapper and accepts all standard HTML div props. No additional custom props are defined.
