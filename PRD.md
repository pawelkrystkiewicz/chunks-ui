...
- **Public adoption.** This is not marketed to the community. No Discord.
- **Good to have** contributor guidelines, "getting started" tutorial. The docs site exists for personal reference and portfolio showcase.
- **Framework-agnostic.** React 19+ only
- **Component kitchen sink.** Not competing with shadcn, Radix Themes, or MUI on component count.
- **Backward compatibility with Creation UI not planned!** API will be redesigned where it makes sense. No adapter layer.
...
Components are ordered by build priority. Tier 1 ships in v0.1.0. Tier 2 lands in v0.x patches. Tier 3 is aspirational.
...
**The "dropdown" problem.** In large design systems (including the one maintained at big corps), the single biggest source of bugs and support tickets is a "dropdown" / "autocomplete" God Component that tries to be Select, Combobox, and Search Box simultaneously. Every bug fix in one mode breaks two others. Forty props, infinite edge cases.
...
v0.1.0 ships Select + Combobox. Autocomplete joins in later if needed.
...
shadcn/ui full compat is expected. Beside that:

```css
@theme inline {
  /* Typography - tailwind modifier*/
  --font-sans: 'Manrope', sans-serif;
  --font-mono: 'Fira Code', monospace;

  /* Easing curves - tailwind modifier */
  --ease-fluid: cubic-bezier(0.3, 0, 0, 1);
  --ease-snappy: cubic-bezier(0.2, 0, 0, 1);

  /* global CSS consts to avoid react Theme Context Providers */
...
| Chart colors                   | 5 chart variables               | Carried forward. shadcn-compatible                           |
| Sidebar tokens                 | Full sidebar palette            | Carried forward. shadcn-compatible                           |

---

## 4. Package Structure

### 4.1 Decision: Lightweight monorepo, single publishable package

**`chunks-ui` is the only npm-published package.** The repo is a minimal Bun workspace monorepo (see [Section 8](#8-repository-structure-decision)) with two workspaces: the UI package and the docs site.

The monorepo exists solely so docs can import `"chunks-ui": "workspace:*"` and see changes live during development.

### 4.2 Package details (`packages/ui/package.json`)
...
- **Semantic versioning** with Conventional Commits:
  - `fix:` → patch (0.1.z)
  - `feat:` → minor (0.y.0)
  - `BREAKING CHANGE:` → major (x.0.0) — **triggered by feat!,refactor! or BREAKING CHANGE in commit message**
...
Decision: Turborepo for docs on vercel, publishes 1 package
